...
type AuthorizationListItem struct {
	...
	// Chain ID this authorization is scoped to, as a decimal string. Uses 256-bit
	// precision, matching `Transaction.value`.
	//
	...
	//
	// Field 1 is reserved rather than retyped in place: `uint64` is a varint and
	// `string` is length-delimited, so reusing the number would let old payloads
	// decode into a new reader as a *wrong* value instead of an absent one.
	// Moving to a fresh number fails safe in both directions — each side sees the
	// field as missing rather than misreading it. The JSON name is unchanged, so
...
const file_models_proto_rawDesc = "" +
	...
	"\achainId\x18\b \x01(\tR\achainIdJ\x04\b\x01\x10\x02\"|\n" +
	"\n" +
	"Withdrawal\x12\x14\n" +
